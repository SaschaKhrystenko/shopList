...
public class Armany extends BaseDAO implements Shop,Runnable  {
    private static Armany instance;
    private Armany(){

    }

    public static synchronized Armany getInstance(){
        if(instance == null){
            instance = new Armany();
        }
        return instance;
    ...
    public int getShopId(String shopName){
        ...
        try {
            ...
            PreparedStatement preStet =connection.prepareStatement("SELECT id FROM shop_1 WHERE name = ?");
    ...
    public List getAllShopData (int shop_id){
        ...
        try {
            ...
            PreparedStatement preStat =connection.prepareStatement("SELECT shop_1.name, category.name, product.title, product.price, product.status \n" +
                    "  FROM shop_1 JOIN category ON shop_1.id = category.shop_id JOIN product ON\n" +
                    "  category.id = product.category_id WHERE shop_1.id = ?;");
